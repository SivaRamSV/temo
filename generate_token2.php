...
$user= $_POST['name'];
$tvaluser=$_POST['usertoken'];
$result = mysqli_query($con,"SELECT UTime,Etime FROM `upload` WHERE usertoken='$tvaluser'");
//$result1 = mysqli_query($con,"SELECT  FROM `upload` WHERE usertoken='$tvaluser'");
$row = mysqli_fetch_assoc($result);
//$row1 = mysqli_fetch_assoc($result1);
$time1=$row['UTime'];
$time2=$row['Etime'];
$hash=password_hash($tvaluser, PASSWORD_DEFAULT);
//echo $time1;
//echo $time2;
...
if(isset($name) && !empty($name))
{
	if( $extension == $size<=$max_size)
	{
		...
		{
			$query = "INSERT INTO `upload` (name, size, type, location, user, token, usertoken, UTime, Etime, hash) VALUES ('$name', '$size', '$type', '$location$name', '$user', '$tval','$tvaluser','$time1','$time2','$hash')";
